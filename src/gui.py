...
class App(ctk.CTk):
    ...
    def _create_widgets(self):
        ...
        self.attach_btn = ctk.CTkButton(self, text="Attach to Game", fg_color="#b71c1c", command=self.attach_game)
        ...
        for name, label in zip(cheat_names, cheat_labels):
            switch = ctk.CTkSwitch(self.cheats_frame, text=label, progress_color="#b71c1c",
                                   command=lambda n=name: self.toggle_cheat(n))
            switch.pack(pady=2, anchor="w")
            self.cheat_switches[name] = switch

        self.start_loop_btn = ctk.CTkButton(self, text="Start Cheat Loop", fg_color="#b71c1c", command=self.start_loop)
        self.start_loop_btn.pack(pady=5)

        self.stop_loop_btn = ctk.CTkButton(self, text="Stop Cheat Loop", fg_color="#b71c1c", command=self.stop_loop)
